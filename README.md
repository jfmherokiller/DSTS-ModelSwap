...
- **Digimon ride** — you can start a Digimon ride while *rendered* as a Digimon (on by default). The mount
  ...
  Temporary-Human ride normally either way.
...
rigs. The generated rows enable `player_change_model`'s field-Analyze capability (`Bool 8`). The
Temporary-Human hotkey flips a "suppress swap" flag and calls the game's own field-model
...
- **Analyze (`Q`) now works directly in Digimon form.** The field gate consumes `Bool 8` from the active
  `player_change_model` row; v1.2.0 enables it for every generated row. The optional
  **Temporary-Human hotkey** remains for other human-only interactions and troubleshooting. See
  [`docs/ANALYZE-GATE.md`](docs/ANALYZE-GATE.md) for the confirmed trace.
